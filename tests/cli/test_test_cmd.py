...
class TestListTests:
    ...
    def test_returns_test_names_sorted(self, tmp_path: Path) -> None:
        scaffold_test(tmp_path, "zebra")
        scaffold_test(tmp_path, "apple")
        result = list_tests(repo_root=tmp_path)
        assert result == ["apple", "zebra"]

    def test_only_includes_directories_with_test_flow_yml(self, tmp_path: Path) -> None:
        scaffold_test(tmp_path, "valid")
        (tmp_path / ".orca" / "tests" / "stray").mkdir()
        result = list_tests(repo_root=tmp_path)
        assert result == ["valid"]
...
class TestResolveTestPaths:
    def test_resolves_paths_for_existing_test(self, tmp_path: Path) -> None:
        scaffold_test(tmp_path, "my-test")
        paths = resolve_test_paths(repo_root=tmp_path, name="my-test")
        assert paths.config_path == (tmp_path / ".orca" / "tests" / "my-test" / "test-flow.yml").resolve()
        assert paths.task_file == (tmp_path / ".orca" / "tests" / "my-test" / "input.md").resolve()
