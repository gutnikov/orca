...
class TestBuildParser:
    ...
    def test_base_without_branch_errors(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        (tmp_path / ".orca").mkdir()
        (tmp_path / ".orca" / "default.yml").write_text("initial: todo")
...
class TestResolveConfigPath:
    def test_default_returns_default_yml(self, tmp_path: Path) -> None:
        orca_dir = tmp_path / ".orca"
        orca_dir.mkdir()
        (orca_dir / "default.yml").write_text("initial: todo")
        assert resolve_config_path(tmp_path, None) == orca_dir / "default.yml"

    def test_workflow_shorthand(self, tmp_path: Path) -> None:
        orca_dir = tmp_path / ".orca"
        orca_dir.mkdir()
        (orca_dir / "develop.yml").write_text("initial: todo")
        assert resolve_config_path(tmp_path, "develop") == orca_dir / "develop.yml"
    ...
    def test_error_lists_available_files(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        orca_dir.mkdir()
        (orca_dir / "default.yml").write_text("initial: todo")
        (orca_dir / "test.yml").write_text("initial: todo")
        with pytest.raises(SystemExit):
            resolve_config_path(tmp_path, "develop")
        captured = capsys.readouterr()
        assert "default.yml" in captured.err
        assert "test.yml" in captured.err
