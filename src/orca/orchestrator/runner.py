...
def resolve_config_path(repo_root: Path, workflow: str | None) -> Path:
    ...
    Args:
        ...
        workflow: Shorthand name (e.g. "develop" -> .orca/develop.yml),
                  a file path (absolute, relative, or ~-prefixed), or None for .orca/default.yml.
    ...
    # Path detection: contains "/" or ends with ".yml"
    if workflow and ("/" in workflow or workflow.endswith(".yml")):
        raw = Path(workflow).expanduser()
        # Absolute (or ~-expanded to absolute) paths resolve directly;
        # relative paths resolve against repo_root.
        config_path = raw.resolve() if raw.is_absolute() else (repo_root / raw).resolve()
        if not config_path.exists():
            print(f"Error: flow file not found: {config_path}", file=sys.stderr)
            raise SystemExit(1)
        return config_path

    orca_dir = repo_root / ".orca"
    config_name = f"{workflow}.yml" if workflow else "default.yml"
    config_path = orca_dir / config_name
    if config_path.exists():
        return config_path

    available = sorted(orca_dir.glob("*.yml")) if orca_dir.exists() else []
    available_str = ", ".join(p.name for p in available) if available else "(none found)"
    print(f"Error: {config_name} not found in {orca_dir}. Available: {available_str}", file=sys.stderr)
    raise SystemExit(1)
...
def _recover_effects(
    ...
    for issue_id, issue in state.issues.items():
        ...
        result_path = (
            worktree_path / ".orca-state" / "result.json" if worktree_path.exists() else run_dir / "result.json"
        )
...
async def run(
    ...
    run_dir = repo_root / ".orca-state" / "runs" / branch_name / workflow
