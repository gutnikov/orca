...
If the user is not in an orca-enabled project yet, skip this — direct them to [orca-create-workflow.md](orca-create-workflow.md). With the orca plugin installed, the `orca-setup` skill auto-triggers on phrases like *"set up orca"* and runs the same end-to-end setup; in environments without the plugin, follow the playbook by reading it directly.
...
- next step: set up a project (invoke the `orca-setup` skill if the orca plugin is installed, otherwise follow [orca-create-workflow.md](orca-create-workflow.md)), or run an existing workflow ([orca-run-workflow.md](orca-run-workflow.md))
