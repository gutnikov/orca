...
| `{{ run.run_dir }}` | string | `.orca-state/runs/BRANCH/WORKFLOW` |
...
**Good:** `values: [done, blocked]` — worker can escalate. Workers can also use the built-in `waiting` outcome for human-in-the-loop.
