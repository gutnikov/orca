...
- [ ] **At least one routable outcome** — every active state has ≥1 outcome with an `on:` rule. A state where no outcome maps to a transition cannot progress.
...
- [ ] **Fail-safe outcomes present** — every active state has `blocked` or some escape hatch beyond the happy-path outcome. Without this, workers report "done" even when stuck. Workers can also use the built-in `waiting` outcome for human-in-the-loop.
...
- [ ] **Conditional sections guarded** — sections that reference optional data (depends_on, children, event_log) use `{% if %}` guards so they don't render as empty headers.
